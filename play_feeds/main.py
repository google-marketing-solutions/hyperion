...
def transfer_play_reports(
    bucket: str, table_suffix: str, project_id: str, dataset_id: str,
  ...
  log_message = ('Transferring reports from Cloud Storage to BigQuery table. '
  ...
  for report_type, dimensions in REPORT_TYPES.items():
    ...
    write_cloud_storage_to_bigquery(
        report_type, dimension_blobs, table_suffix, project_id, dataset_id,
        start_date, end_date, encoding='utf-16', dry_run=dry_run)


def write_cloud_storage_to_bigquery(
    report_type: str, dimension_blobs: Mapping[str, list[Blob]],
    table_suffix: str, project_id: str, dataset_id: str,
    start_date: datetime, end_date: datetime,
    encoding: str = 'utf-16', dry_run: bool = False):
  ...
  def get_date_fields_by_report_type(report_type):
    if report_type == 'Reviews':
      return ['Review Submit Date and Time',
  ...
  for dimension, blobs in dimension_blobs.items():
    df = gcs_utils.get_blobs_data_frame_with_date_range(
        blobs, start_date, end_date, dry_run=dry_run, date_fields=date_fields)
    ...
    if not df.empty:
      ...
      if (bq_utils.table_exists(client, table_id) and
          bq_utils.table_has_same_data_for_date_range(
              num_rows, client, table_id,
              start_date_string, end_date_string, date_filter_fields)):
        logger.log(
    ...
    for game, bucket_suffix in mappings.items():
      ...
      if bucket and table_suffix:
        transfer_play_reports(
            bucket, table_suffix, project_id, dataset_id, start_date, end_date,
            encoding='utf-16', dry_run=dry_run)
      else:
        logger.log(
...
def backfill(event: Mapping[str, Mapping], context: Mapping):
  ...
  # report_types = pubsub_attributes.get('report_types')

  if dry_run == 'dry_run':
    logger.log('Running backfill test...')
    run(dry_run=True, start_date=start_date, end_date=end_date)
  else:
